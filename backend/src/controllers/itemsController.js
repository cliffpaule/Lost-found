...
const updateItem = async (req, res) => {
  try {
    ...
    const { title, description, category_id, location_id, location_detail,
            date_occurred, status, is_valuable, reward_offered, reward_amount } = req.body;
    ...
    `, [
      ...
      reward_amount || item.reward_amount, item.id
    ]);
...
const getStats = async (req, res) => {
  try {
    ...
    const [todayItems] = await query("SELECT COUNT(*) as count FROM items WHERE DATE(created_at) = CURDATE()");
...
const claimItem = async (req, res) => {
  try {
    ...
    await query(
      'INSERT INTO claims (item_id, claimant_id, proof_description, proof_image_url) VALUES (?, ?, ?, ?)',
    ...
    res.json({ success: true, message: 'Claim submitted successfully. We will review and notify you.' });
  } catch (err) {
    res.status(500).json({ success: false, message: 'Failed to submit claim' });
  }
};

module.exports = { getItems, getItemById, createItem, updateItem, deleteItem, getStats, claimItem };
